...
for i in range(size_par):
    ...
    model_best, val_loss_history, train_loss_history = train_model(model, dataloaders, loss_function, optimizer, num_epochs=3)
...
if flag_target_as_feat == 1:
    feat_target = 'yes'
else:
    feat_target = 'no'
if flag_single_speaker == 1:
    side = 'one_side'
else:
    side = 'two_side'
